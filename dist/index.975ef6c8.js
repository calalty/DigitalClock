...
class Clock {
    constructor(currentTimeText1){
        this.currentTimeText = currentTimeText1;
        this.getTime();
    }
    getTime() {
    ...
    localeTimeString(input, timezone1) {
        this.currentTime = this.current.toLocaleTimeString(`${input}`, {
            timeZone: timezone1
        });
    }
    ...
    changeTimezone(timezone2) {
        ...
        this.timezones.forEach((i)=>{
            if (this.timezone == i) this.localeTimeString(this.time, this.timezone);
        });
    }
    showTime() {
...
timezoneArray.forEach((el, index, inputArray)=>{
    if (inputArray.indexOf(el) == index) timezone.add(new Option(el));
});
timezones.forEach((e)=>{
...
},{}]},["ShInH","8lqZg"], "8lqZg", "parcelRequire98fc")
